...
export function BottomNav() {
  ...
  return (
    <nav 
      className="fixed bottom-0 left-0 right-0 z-[1000] bg-white border-t border-gray-100 flex flex-col shadow-[0_-4px_20px_rgba(0,0,0,0.04)] w-full"
      style={{ 
        paddingBottom: 'env(safe-area-inset-bottom)' 
      }}
    >
      <div className="flex items-center w-full h-[50px] px-1">
        {navItems.map((item) => {
            ...
            >
              <div className="flex items-center justify-center h-5 w-5 relative">
                ...
                {item.hasBadge && cartCount > 0 && (
                  <span className="absolute -top-1.5 -right-2.5 bg-primary text-white text-[7px] font-black h-3.5 w-3.5 rounded-full flex items-center justify-center border border-white animate-in zoom-in">
                    {cartCount}
                  </span>
                )}
              </div>
