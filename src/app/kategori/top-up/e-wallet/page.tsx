      ...
      <main className="pt-20 px-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <Input 
                ...
                value={walletNumber}
                onChange={(e) => {
                  setWalletNumber(e.target.value.replace(/[^0-9]/g, ''));
                  if (errors.wallet) setErrors({...errors, wallet: undefined});
                }}
                className={cn(
                  "rounded-2xl h-12 text-base md:text-sm font-semibold border-gray-100 focus-visible:ring-primary/20 pl-4",
                  errors.wallet && "border-red-500 bg-red-50/30"
                )}
              />
              {errors.wallet && (
                <p className="text-[10px] text-red-500 font-bold mt-1 ml-1 flex items-center gap-1">
                  <AlertCircle className="w-3 h-3" /> {errors.wallet}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-sm font-bold text-gray-800">Pilih Nominal Saldo {selectedProvider.name}</h2>
          </div>
            ...
            {selectedProvider.products.map((product) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-gray-800 tracking-tight">SALDO {product.nominal}</p>
        ...
        <div className="mt-8 bg-blue-50/50 p-4 rounded-2xl border border-blue-100 flex gap-3">
          ...
          <p className="text-[10px] text-blue-700 leading-relaxed font-medium">
            Saldo akan terkirim secara otomatis setelah pembayaran terverifikasi. Pastikan Nomor E-Wallet dan Nama Pemilik sudah sesuai.
          </p>
        </div>
      </main>
