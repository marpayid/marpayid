        ...
        <section className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm mb-4 space-y-4">
          <div className="space-y-1.5">
            ...
            <Input 
              ...
              value={customerName} 
              onChange={(e) => { setCustomerName(e.target.value); if (errors.name) setErrors({...errors, name: undefined}); }} 
              className={cn("rounded-2xl h-12", errors.name && "border-red-500")} 
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input 
              ...
              value={walletNumber} 
              onChange={(e) => { setWalletNumber(e.target.value.replace(/[^0-9]/g, '')); if (errors.wallet) setErrors({...errors, wallet: undefined}); }} 
              className={cn("rounded-2xl h-12", errors.wallet && "border-red-500")} 
            />
          </div>
        </section>

        {/* Pemilihan E-Wallet - Diperbaiki menjadi 1 baris tanpa scroll */}
          ...
          <div className="grid grid-cols-4 gap-2">
            {PROVIDERS.map((provider) => (
              <button 
                ...
                className={cn(
                  "flex flex-col items-center justify-center p-2 rounded-2xl border transition-all gap-1.5", 
                  selectedProvider.id === provider.id ? "border-primary bg-primary/5 text-primary shadow-sm" : "border-gray-50 text-gray-400"
                )}
              >
                <div className={cn(
                  "w-9 h-9 rounded-full flex items-center justify-center transition-colors", 
                  selectedProvider.id === provider.id ? "bg-primary text-white" : "bg-gray-100"
                )}>
                  <Wallet className="w-5 h-5" />
                </div>
                <span className="text-[7.5px] font-black uppercase text-center leading-none tracking-tighter">
                  {provider.name}
                </span>
              </button>
            ))}
